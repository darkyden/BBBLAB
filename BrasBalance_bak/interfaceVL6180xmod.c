...
volatile int flag_detM = 0; // 0: pas d'objet détecté, 1: objet détecté
//définitions privée
typedef struct
{
	int adresse;
...
uint8_t message[3];
	...
	message[2] = Donnee;
        ...
        if(write(fd, &message, NombreDOctetsAEcrire) != NombreDOctetsAEcrire)
        {
...
int interfaceVL6180x_litUneDistance(float *Distance)
{
     ...
     if (flag_detM) {
        printf("Mesure arrêtée : objet déjà détecté.\n");
        return -1; // Arrête la prise de mesure si le flag est actif
    }
